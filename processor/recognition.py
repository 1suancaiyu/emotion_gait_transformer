    ...
    def test(self, evaluation=True, testing_A=False, save=False, save_feature=False):
        ...
        if testing_A:
            ...
            if evaluation:
                ...
                #self.epoch_info['mean_loss1'] = np.mean(loss1_value)
                self.epoch_info['mean_loss_class'] = np.mean(loss_class_value)
                self.epoch_loss_class_eval.append(np.mean(loss_class_value))
                #self.epoch_info['mean_loss_recon'] = np.mean(loss_recon_value)
                self.show_epoch_info()
                ...
                # wsx eval curve
                plt.figure()
                epoch_x = np.arange(0, len(self.epoch_loss_class_eval)) + 1
                plt.plot(epoch_x, self.epoch_loss_class_eval[0:], '--', color='C1')
                plt.legend(['epoch_loss_class_eval'])
                plt.ylabel('action CrossEntropyLoss')
                plt.xlabel('Epoch')
                plt.xlim(0, len(self.epoch_loss_class_eval))
                plt.savefig('epoch_loss_class_eval.png')
                plt.close()

                with open("epoch_loss_class_eval.txt", "w") as outfile:
                    for item in self.epoch_loss_class_eval:
                        outfile.write("{}: {}\n".format(self.epoch_info, item))
