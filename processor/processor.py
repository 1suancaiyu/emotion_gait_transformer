...
class Processor(IO):
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            ...
            model3_wegiht_dir =  './work_dir/recognition/ntu-xsub/AS_GCN/max_hop_4/lamda_05/epoch54_model3.pt'
            self.model3 = self.io.load_weights(self.model3, model3_wegiht_dir, self.arg.ignore_weights)
            #self.io.print_log('Model:   {}.'.format(self.arg.model2))
            #self.io.print_log('Weights: {}.'.format(self.arg.weights2))
            self.io.print_log('Model:   {}.'.format('model3'))
            self.io.print_log('Weights:   {}.'.format(model3_wegiht_dir))
